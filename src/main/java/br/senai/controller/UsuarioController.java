...
public class UsuarioController {
    ...
    public ResponseEntity<Usuario> create(@RequestBody Usuario usuario)
        ...
        if (createdUsuario == null){ return ResponseEntity.notFound().build(); }
        else {
    ...
    public ResponseEntity<Usuario> update(@RequestBody Usuario usuario, @PathVariable Long id){
        ...
        if (updatedUsuario == null){ return ResponseEntity.notFound().build(); }
        else { return ResponseEntity.ok(updatedUsuario); }
    }
    ...
    public ResponseEntity<Optional<Usuario>> read(@PathVariable Long id){
        ...
        if(foundUsuario.isEmpty()){ return ResponseEntity.notFound().build(); }
        else { return ResponseEntity.ok(foundUsuario); }
    }

    @GetMapping(value = "/", produces = "application/json")
    public ResponseEntity<List<Usuario>> findAll(){
        List<Usuario> allUsuarios = usuarioService.findAll();
        if(allUsuarios.isEmpty()){ return ResponseEntity.notFound().build(); }
        else { return ResponseEntity.ok(allUsuarios); }
    }
